...
describe('publish calculations', () => {
  ...
  describe('When multiple calculations are unpublished', () => {
    ...
    // describe('When staggered amount of calculations and fundings are unpublished', () => {
    //   beforeEach(async () => {
    //     publishingConfig.dataPublishingMaxBatchSize = 5
    //     await db.sequelize.truncate({ cascade: true })
    //   })
    ...
    //     const numberOfRecordsCalc = 1
    //     const numberOfRecordsFund = -1 + publishingConfig.dataPublishingMaxBatchSize
    //     await db.calculation.create(mockCalculation1)
    //     await db.funding.bulkCreate([...Array(numberOfRecordsFund).keys()].map(x => { return { ...mockFunding1, fundingId: mockFunding1.fundingId + x, calculationId: mockCalculation1.calculationId } }))
    //     const unpublishedBefore = await db.calculation.findAll({ where: { published: null } })

    //     await publish()

    //     const unpublishedAfter = await db.calculation.findAll({ where: { published: null } })
    //     expect(unpublishedBefore).toHaveLength(numberOfRecordsCalc)
    //     expect(unpublishedAfter).toHaveLength(0)
    //   })
    ...
    //     const numberOfRecordsCalc = 1
    //     const numberOfRecordsFund = publishingConfig.dataPublishingMaxBatchSize
    //     await db.calculation.create(mockCalculation1)
    //     await db.funding.bulkCreate([...Array(numberOfRecordsFund).keys()].map(x => { return { ...mockFunding1, fundingId: mockFunding1.fundingId + x, calculationId: mockCalculation1.calculationId } }))
    //     const unpublishedBefore = await db.calculation.findAll({ where: { published: null } })
